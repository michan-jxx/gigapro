...
<body>
    <x-app-layout>
        ...
        <div class="form">
            ...
            <div class="template">
                <a href = "{{ route('admin.pc.download') }}">
                    <button class ="btnn">テンプレートをダウンロード</button>
                </a>
            </div>

            <form method="POST" action="{{ route('admin.pc.import') }}" enctype="multipart/form-data">
                @csrf
                @if ($errors->any())
            ...
            <div class="back">
                <input type="button" name="back" value="戻る" onclick="history.back()" class="back">
            </div>
        </div>
    </x-app-layout>
</body>
